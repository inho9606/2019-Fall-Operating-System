...
//	else {
//	printf("%d thread id create\n", tid);
//	sema_down(&(thread_current()->wait_for_load));
//	sema_up(&(thread_current()->pause2));
//	}
	return tid;
}
...
//	printf("%s name %d thread id\n", thread_current()->name, thread_current()->tid);
//	sema_up(&((thread_current()->parent)->wait_for_load));
//	sema_down(&(thread_current()->pause2));
...
//	printf("%s current thread\n", parent->name);
	for(ptr = list_begin(&parent->children); ptr != list_end(&parent->children); ptr = ptr->next) {
		child = list_entry(ptr, struct thread, child_elem);
//		printf("child %s process %d exit\n", child->name, child->tid);
		if(child->tid == child_tid) {
//		printf("%s thread waits for %s thread\n", parent->name, child->name);
			sema_down(&(child->wait_for_exit));
			i = child->exit;
//			list_remove(ptr);
...
load(const char* file_name, void (**eip) (void), void** esp)
{
	...
	off_t file_ofset;
	bool success = false;
	int i, cnt = 0, stacksize = 0;
	char userstack[40][20];
	uint32_t stackaddress[40];
	char* token, * ptr;
	...
	if (t->pagedir == NULL)
		goto done;
	process_activate();
	token = strtok_r(file_name, " \0", ptr);
	while (token != NULL) {
		strlcpy(userstack[cnt++], token, strlen(token) + 1);
	...
	{
		for (i = cnt - 1; i >= 0; i--) {
			*esp -= strlen(userstack[i]) + 1;
			memcpy(*esp, &userstack[i], strlen(userstack[i]) + 1);
			stackaddress[i] = *esp;
			stacksize += strlen(userstack[i]) + 1;
		}
		*esp -= 4 - (stacksize % 4);
